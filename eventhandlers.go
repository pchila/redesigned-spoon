...
// HandleGetSliTriggeredEvent handles get-sli.triggered events if SLIProvider == redesigned-spoon
...
func HandleGetSliTriggeredEvent(myKeptn *keptnv2.Keptn, incomingEvent cloudevents.Event, data *keptnv2.GetSLITriggeredEventData) error {
	...
	if data.GetSLI.SLIProvider != "redesigned-spoon" {
	...
	// Get SLI File from redesigned-spoon subdirectory of the config repo - to add the file use:
	//   keptn add-resource --project=PROJECT --stage=STAGE --service=SERVICE --resource=my-sli-config.yaml  --resourceUri=redesigned-spoon/sli.yaml
	sliFile := "redesigned-spoon/sli.yaml"
